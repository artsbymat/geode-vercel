...
var (
	...
	embedWikiLinkReg = regexp.MustCompile(`\!\[\[(.*?)\]\]`)
...
func StripMarkdown(s string) string {
	...
	res = wikiLinkReg.ReplaceAllString(res, "$1")
	res = embedWikiLinkReg.ReplaceAllString(res, "")
	res = emptyLineReg.ReplaceAllString(res, "")
